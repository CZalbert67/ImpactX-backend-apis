...
| 15 | Health checks | `GET /health` |
| 16 | OpenAPI | `/openapi/v1.json` + Scalar UI (solo Development) |
...
|---|---|---|---|
| 1 | `/health/live` y `/health/ready` | Solo existe `GET /health` genérico | Necesario para Kubernetes/orquestación |
| 2 | Versionado de API (`/api/v1/`) | Rutas como `api/auth`, `api/users` | Dificulta evolución sin romper clientes |
| 3 | `X-Correlation-Id` | No implementado | Sin trazabilidad de extremo a extremo |
| 4 | `traceparent` / OpenTelemetry | ASP.NET Core reconoce `traceparent` internamente, pero no hay integración explícita de trazabilidad distribuida ni OpenTelemetry | Sin visibilidad de tracing entre servicios |
| 5 | Problem Details (RFC 7807) | Usa `ErrorResponse` custom | No sigue estándar `application/problem+json` |
| 6 | Fechas como `DateTimeOffset` | Todas las fechas son `DateTime` | Se pierde información de zona horaria |
| 7 | Swagger UI | Actualmente usa **Scalar** (`Scalar.AspNetCore` v2.*). El requisito del profesor es exponer `/swagger` solo en Development | Migrar a Swashbuckle o configurar `/swagger` además de Scalar |
| 8 | Rate limiting | No implementado | Riesgo en endpoints públicos (login/register) |
| 9 | CSRF | No implementado. Depende del método de almacenamiento y transporte del JWT | Evaluar según la estrategia de almacenamiento del token |
| 10 | `IEmailService` real | Solo existe `StubEmailService` | Sin envío real de correos |
...
| 8 | `/health/live` + `/health/ready` | Ninguna |
| 9 | Actualizar tests de integración para reflejar cambios en rutas y formato de errores | Pasos 5, 7 |
| 10 | Sustituir o complementar Scalar con Swagger UI (`/swagger`) solo en Development | Ninguna |
...
| `AGENTS.md` | ✅ Modificado (regla de ramas actualizada) |
